import os
import random
import json
import torch
import cv2
import numpy as np
import torch.nn.functional as F
from torch.utils.data import Dataset
import torchvision.transforms as transforms
from PIL import Image, ImageDraw, ImageOps

def ResizeImage(target_height, image, fill_number=255, max_width=None):
    original_width,  original_height = image.size
    new_width = int(original_width * (target_height / original_height))
    image_resized = image.resize((new_width, target_height))
    fill_number = (fill_number,fill_number,fill_number)
    remainder = new_width % 16
    if remainder != 0:
        pad_total = 16 - remainder
        pad_left = pad_total // 2
        pad_right = pad_total - pad_left
        image_resized = ImageOps.expand(image_resized, border=(pad_left, 0, pad_right, 0), fill=fill_number)
    cur_width = image_resized.size[0]
    if max_width is not None:
        if cur_width>max_width:
            start_x = random.randint(0, cur_width - max_width)
            image_resized = image_resized.crop((start_x, 0, start_x + max_width, target_height))
    image_resized = ImageOps.expand(image_resized, border=(16, 0, 16, 0), fill=fill_number)
    new_width = image_resized.size[0]
    return image_resized, new_width


class TextLineDataset(Dataset):
    def __init__(self, args, mode, phase):
        super().__init__()
        self.root = args.data_root + '/data_ch'
        self.phase = phase
        self.mode = mode
        self.target_height = args.height
        self.reference_num = args.reference_num
        self.template_root = args.data_root + '/data_temp_ch'

        if mode == 'chinese':
            if self.phase == 'train':
                self.data_path = os.path.join(self.root,'train')
                writer_id_list = list(os.listdir(self.data_path))
                self.writer_id = sorted(writer_id_list, key=lambda x: int(x))
                self.writer_num = len(self.writer_id)
                self.max_width = args.width
            if self.phase == 'test':
                self.data_path = os.path.join(self.root,'test')
                writer_id_list = list(os.listdir(self.data_path))
                self.writer_id = sorted(writer_id_list, key=lambda x: int(x[1:]))
                self.writer_num = len(self.writer_id)
                self.max_width = None
        self.get_path()
    
    def get_path(self):
        self.target_images = []
        self.template_images = []
        self.style_images = {}

        self.style_list = []
        self.label_list = []
        self.id_list = []

        for style in self.writer_id:
            images_related_style = []
            passage_path = os.path.join(self.data_path,style)
            for passage in os.listdir(passage_path):
                line_path = os.path.join(passage_path,passage)
                for img in os.listdir(line_path):
                    img_path = os.path.join(line_path,img)
                    img_name = img.split('.')[0]
                    self.target_images.append(img_path)
                    template_path = os.path.join(self.template_root, self.phase, style, passage, img_name+"_template.png")
                    self.template_images.append(template_path)
                    images_related_style.append(img_path)
                    self.style_list.append(style)
            self.style_images[style] = images_related_style
        
    def __getitem__(self, index):
        target_image_path = self.target_images[index]
        content_image_path = self.template_images[index]
        target_image_name = target_image_path.split('/')[-1].split('.')[0]
        style = self.style_list[index]

        origin_image = Image.open(target_image_path).convert("RGB")
        origin_image, origin_img_width = ResizeImage(self.target_height, origin_image, max_width=self.max_width)

        content_image = Image.open(content_image_path).convert('RGB')
        content_image, _ = ResizeImage(self.target_height,content_image, max_width=self.max_width)

        images_related_style = self.style_images[style].copy()
        if target_image_path in images_related_style:
            images_related_style.remove(target_image_path)
        else:
            raise ValueError(f"Error: {target_image_path} not found in images_related_style.")

        style_image_path = random.choice(images_related_style)
        style_image = Image.open(style_image_path).convert("RGB")
        style_image, style_image_width = ResizeImage(self.target_height,style_image, max_width=self.max_width)

        sample = {
            "content_image": content_image,
            "style_image": style_image,
            "origin_image": origin_image,
            "origin_img_width": origin_img_width,
            "style_image_width": style_image_width,
            "target_image_path": target_image_path,
            "style":style,
            "target_image_name":target_image_name,
        }
        return sample

    def __len__(self):
        return len(self.target_images)

