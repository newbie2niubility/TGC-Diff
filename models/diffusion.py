...
class Diffusion:
    ...
    ## output
    def train_ddim(self, model, x, style, content, total_t, sampling_timesteps=5, eta=0):
        ...
        style_feature = model.module.style_encoder(style)
        vae_content = model.module.content_encoder(content,None)
        ...
        for time, time_next in tqdm(time_pairs, position=1, leave=False, desc='sampling'):
            ...
            predicted_noise = model.module.unet_model(x, timesteps=time, vae_content=vae_content, context=style_feature)
    ...
    @torch.no_grad()
    def ddim_sample(self, model, vae, n, x, style, content, sampling_timesteps=50, eta=0):
        model.eval()
        vae.eval()
        mid_latent = []
        ...
        time_pairs = list(zip(times[:-1], times[1:])) # [(T-1, T-2), (T-2, T-3), ..., (1, 0), (0, -1)]
        x_start = None
        vae_content = vae.encode(content).latent_dist.sample()
        ...
        for time, time_next in tqdm(time_pairs, position=1, leave=False, desc='sampling'):
            time = (torch.ones(n) * time).long().to(self.device)
            time_next = (torch.ones(n) * time_next).long().to(self.device)
            predicted_noise = model.module.unet_model(x, time, vae_content=vae_content, context=style_feature)
