...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.conv_in1 = nn.Conv2d(in_channels, model_channels//2, kernel_size=3, padding=(1, 1))
        self.conv_in2 = nn.Conv2d(in_channels, model_channels//2, kernel_size=3, padding=(1, 1))
    ...
    def forward(self, x, timesteps=None, vae_content=None, context=None, **kwargs):
        """
        ...
        h = x.type(self.dtype)
        h = self.conv_in1(h)
        vae_content = self.conv_in2(vae_content)
        h = torch.cat((h,vae_content),dim=1)
...
##################################################################################
class ImitatingDiff(nn.Module):
    ### A model that includes both the UNet and StyleEncoder as separate modules.
    def __init__(
        self,
        in_channels,
        model_channels,
        ...
        super().__init__()
        self.unet_model = UNetModel(in_channels=in_channels, model_channels=model_channels, 
                     out_channels=out_channels, num_res_blocks=num_res_blocks, 
                     attention_resolutions=attention_resolutions, channel_mult=channel_mult, num_heads=num_heads, 
                     context_dim=context_dim)
        self.style_encoder = StyleEncoder(d_model=context_dim, num_encoder_layers=style_encoder_layers)

    def forward(self, x, timesteps=None, style=None, content=None, **kwargs):
        context = self.style_encoder(style)
        output = self.unet_model(x, timesteps=timesteps, vae_content=content, context=context, **kwargs)
